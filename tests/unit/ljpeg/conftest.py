...
def good_example_numpy_lossless_photo_w_decoded(data_folder_path):
    input_file = np.fromfile(str(data_folder_path / 'F-18.ljpg'), np.uint8).astype(np.intc)
    expected = np.load(str(data_folder_path / 'F-18.ppm.pickle.npy'))
    return input_file, expected


@pytest.fixture()
def example_numpy_lossy_photo(data_folder_path):
    return np.fromfile(str(data_folder_path / 'lossy.jpg'), np.uint8).astype(np.intc)


@pytest.fixture()
def example_numpy_lossless_photo_w_bad_huffman_value(data_folder_path):
    return np.fromfile(str(data_folder_path / 'F-18_bad_huffman_value.ljpg'), np.uint8).astype(np.intc)


@pytest.fixture()
def example_numpy_lossless_photo_w_bad_hi_value(data_folder_path):
    return np.fromfile(str(data_folder_path / 'F-18_bad_Hi.ljpg'), np.uint8).astype(np.intc)


@pytest.fixture()
def example_numpy_lossless_photo_w_bad_vi_value(data_folder_path):
    return np.fromfile(str(data_folder_path / 'F-18_bad_Vi.ljpg'), np.uint8).astype(np.intc)


@pytest.fixture()
def example_numpy_lossless_photo_w_bad_tqi_value(data_folder_path):
    return np.fromfile(str(data_folder_path / 'F-18_bad_Tqi.ljpg'), np.uint8).astype(np.intc)
